...
(define and->if (lambda (exp)
	(define iter (lambda (exp)
		(if (null? exp)
			'true
...
(define let-clauses (lambda (exp) (cdr exp)))

(define let->combination (lambda (exp)
	(expand-let-clauses (let-clauses exp))))

(define expand-let-clauses (lambda (clauses)
	(let ((binds (car clauses))
		(body (cdr clauses)))
		(if (null? binds)
			'()
			(cons 
